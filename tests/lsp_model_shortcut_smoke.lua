...
local function assert_text_edit(item, filter_text, new_text)
	...
	if item.textEdit.newText ~= new_text then
		fail(
			("textEdit newText mismatch: expected %s, got %s"):format(
				vim.inspect(new_text),
				vim.inspect(item.textEdit.newText)
			)
		)
	end
end
...
local function native_completion_items(client_id, line)
	...
	if result.isIncomplete ~= true then
		fail(
			"native conversion got a complete model shortcut list for "
				.. vim.inspect(line)
				.. ": "
				.. vim.inspect(result)
		)
	...
	return vim.lsp.completion._convert_results(line, 0, #line, client_id, word_boundary, nil, result, "utf-16")
end
...
local function assert_native_completion(client_id, line, expected_labels)
	...
	if not vim.deep_equal(actual, expected_labels) then
		fail(
			("native completion labels mismatch for %s\n  expected: %s\n  actual:   %s\n  items:    %s"):format(
				vim.inspect(line),
...
local function assert_native_expansion(client_id, line, label, expected_word)
	...
	if not item then
		fail(
			"native completion did not include "
				.. vim.inspect(label)
				.. " for "
				.. vim.inspect(line)
				.. ": "
	...
	if word ~= expected_word then
		fail(
			("native completion word mismatch for %s\n  expected: %s\n  actual:   %s\n  item:     %s"):format(
				vim.inspect(line),
