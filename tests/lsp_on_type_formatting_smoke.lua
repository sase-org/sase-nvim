...
local function type_open_paren_stay_insert(expected)
	feed("a(")
	wait_for_line(expected)
	return vim.api.nvim_win_get_cursor(0)
end

local function type_at_checked_cursor(text)
...
set_line("#foo::  ")
cursor = type_open_paren_stay_insert("#foo()::  ")
if cursor[2] ~= #"#foo" then
	fail("double-colon paired cursor was not between parens: " .. vim.inspect(cursor))
end
type_at_checked_cursor("foo=bar")
wait_for_line("#foo(foo=bar)::  ")
feed("<Esc>")
vim.wait(1000, function()
	return vim.api.nvim_get_mode().mode ~= "i"
end, 20)
...
cursor = type_open_paren_stay_insert("#foo()::  ")
if cursor[2] ~= #"#foo" then
	fail("double-colon unpaired cursor was not between parens: " .. vim.inspect(cursor))
end
type_at_checked_cursor("foo=bar")
wait_for_line("#foo(foo=bar)::  ")
feed("<Esc>")
